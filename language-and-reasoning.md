...
Language is much more abstract than prototypical examples of tools like sticks and hammers. It exists in many different forms, as written language, spoken language, recordings of spoken language, language inside people's heads, and so forth. We speak of "a word" as a singular thing, but a word, say "apple," exists either in many different places, or perhaps nowhere, making words peculiar kinds of things, if indeed words are things.

"Language" refers not just to words and other linguistic artifacts, but also to the skill sets of the animals that are able to produce and derive meaning from language, as shown in phrases like "language competence," "language acquisition," etc. Indeed, it's not possible to understand human language without understanding the entire surrounding social context, and the natural world in which the social context exists. Language is inextricably embedded, ultimately, in the planet Earth and the phenomena which occur (primarily) in the film between the atmosphere and the crust.

There are therefore many different forms of analysis with which we can understand language, including analyzing the biology of the organs that produce speech; the neuroscience of the brain functions that control speech; the psychology of language use; the genetic and environmental factors that give rise to language capabilities; the social dynamics and unstated social rules of language use; the social functions of language use; the syntactic structure of speech; the grammar and vocabulary of a language; the politics and power relations of language use in different contexts; the historical development of languages across time and regions; the taxonomic and evolutionary relationships of different languages; and, I'm sure, many others.

One might argue, therefore, that a significant difference between language and prototypical examples of technology is that technology is a tangible product which exists in the material world, whereas language is a socially constructed mass causal web which is variously abstract and concrete on different planes of analysis.
...
Recall that a prime number is a number which is divisible only by itself and one. Suppose, towards a contradiction, that there are finitely many prime numbers. Let x be the number resulting from adding together all the prime numbers and adding one. Since x is evidently larger than all prime numbers, x is not prime. That means that x is divisible by at least one prime number. Pick a prime that x is divisible by, and call it p. We know that the remainder of dividing x by p is 0, since that's what it means for x to be divisible by p. However, we also know that the remainder of dividing p by x is 1, because of how we produced x.

Why is the last sentence true? We produced x by adding up all the (finitely many) prime numbers 

We've seen a couple examples of good arguments. What makes an argument good? Chiefly, two things. First, that it is persuasive: it causes people to believe its conclusion, the more reliably the better. Second, that it is truth-tending: its conclusion is probably going to be true, with whatever level of confidence is required by the situation. Neither of these is more important than the other; both are requirements. In particular, a persuasive argument for a false conclusion is worse than nothing.
